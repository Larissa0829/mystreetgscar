	...
	const bool debug)
{
  ...
  return std::make_tuple(rendered, out_color, out_depth, out_alpha, out_semantic, pixels, radii, geomBuffer, binningBuffer, imgBuffer);
}
	...
	const bool debug) 
{
  ...
  torch::Tensor depth_pixel = torch::full({P, 1}, 0.0, means3D.options());
  ...
  {  
	  ...
	  depth_pixel.contiguous().data<float>(),
	  debug);
  }
//   if (dL_dsemantic.size(1) == 0) 
//   {
//     std::cout << "Warning: No semantic information, setting default values!" << std::endl;
//     dL_dsemantic = torch::zeros({P, S}, means3D.options());  // 或者根据需要设置默认值
// }

// std::cout << "dL_dmeans2D size: " << dL_dmeans2D.sizes() << std::endl;
// std::cout << "dL_dcolors size: " << dL_dcolors.sizes() << std::endl;
// std::cout << "dL_dopacity size: " << dL_dopacity.sizes() << std::endl;
// std::cout << "dL_dmeans3D size: " << dL_dmeans3D.sizes() << std::endl;
// std::cout << "dL_dcov3D size: " << dL_dcov3D.sizes() << std::endl;
// std::cout << "dL_dsh size: " << dL_dsh.sizes() << std::endl;
// std::cout << "dL_dscales size: " << dL_dscales.sizes() << std::endl;
// std::cout << "dL_drotations size: " << dL_drotations.sizes() << std::endl;
// std::cout << "dL_dsemantic size: " << dL_dsemantic.sizes() << std::endl;
// std::cout << "depth_pixel size: " << depth_pixel.sizes() << std::endl;
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations, dL_dsemantic, depth_pixel);
}
