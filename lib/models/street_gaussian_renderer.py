...
class StreetGaussianRenderer():
    ...
    def render_object(
        ...
        result = self.render_kernel(viewpoint_camera, pc, convert_SHs_python, compute_cov3D_python, scaling_modifier, override_color, white_background=True, depth_threshold=depth_threshold)
    ...
    def render_background(
        ...
        include_list = list(set(['background']) - set(exclude_list))
        pc.set_visibility(include_list)
        ...
        if pc.include_sky:
            ...
            sky_color = sky_color * (1 - result['acc'])
            result['rgb'] = result['rgb'] * result['acc'] + sky_color

        if pc.use_color_correction:
            result['rgb'] = pc.color_correction(viewpoint_camera, result['rgb'])
            sky_color = pc.color_correction(viewpoint_camera, sky_color)
    ...
    def render_sky(
        ...
        pc.parse_camera(viewpoint_camera)
        result = self.render_kernel(viewpoint_camera, pc, convert_SHs_python, compute_cov3D_python, scaling_modifier, override_color, depth_threshold=depth_threshold)
        return result
    ...
    def render_kernel(
        ...
        rasterizer = make_rasterizer(viewpoint_camera, pc.max_sh_degree, bg_color, scaling_modifier, depth_threshold)
        ...
        rendered_color, radii, rendered_depth, rendered_acc, rendered_feature, pixels = rasterizer(
            means3D = means3D,
            means2D = means2D,
            opacities = opacity,
            shs = shs,
