...
class GaussianModelActor(GaussianModel):
    ...
    def densify_and_prune(self, max_grad, min_opacity, prune_big_points):
        ...
        if prune_big_points:
            ...
            # Check if samples_xyz is empty before reshaping
            if samples_xyz.numel() > 0:  # Ensure the tensor has elements
                ...
                points_inside_box = torch.zeros(num_gaussians, dtype=torch.bool, device=samples_xyz.device)
