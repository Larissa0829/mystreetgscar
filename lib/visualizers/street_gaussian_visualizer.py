...
class StreetGaussianVisualizer():
    ...
    def visualize(self, result, camera: Camera, mode = None, difixPipe = None, difixPrompt = None):
        ...
        if self.save_image:
            if mode is None:
                ...
                bkgd_dir = os.path.join(final_path,"bkgd")
                bkgd_acc_dir = os.path.join(final_path,"bkgd_acc")
                obj_dir = os.path.join(final_path,"obj")
                os.makedirs(gt_dir, exist_ok=True)
                os.makedirs(render_dir, exist_ok=True)
                os.makedirs(bkgd_dir, exist_ok=True)
                os.makedirs(bkgd_acc_dir, exist_ok=True)
                os.makedirs(obj_dir, exist_ok=True)
                torchvision.utils.save_image(rgb, os.path.join(render_dir, f'{name}_rgb.png'))
                torchvision.utils.save_image(rgb_bkgd, os.path.join(bkgd_dir, f'{name}_rgb_bkgd.png'))
                torchvision.utils.save_image(rgb_obj, os.path.join(obj_dir, f'{name}_rgb_obj.png'))
                # torchvision.utils.save_image(acc_obj.float(), os.path.join(final_path, f'{name}_acc_obj.png'))
                # torchvision.utils.save_image(acc_rgb.float(), os.path.join(final_path, f'{name}_acc_rgb.png'))
                torchvision.utils.save_image(camera.original_image[:3], os.path.join(gt_dir, f'{name}_gt.png'))
                # torchvision.utils.save_image(rgb_sky, os.path.join(final_path, f'{name}_rgb_sky.png'))
                # torchvision.utils.save_image(acc_sky, os.path.join(final_path, f'{name}_acc_sky.png'))
                torchvision.utils.save_image(acc_bkgd.float(), os.path.join(bkgd_acc_dir, f'{name}_acc_bkgd.png'))
